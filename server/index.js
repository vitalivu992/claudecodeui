        ...
        try {
          ...
          shellProcess.onData((data) => {
            if (ws.readyState === ws.OPEN) {
              ...
              // Remove control characters before pattern matching
              ...
              const sanitizedData = stripControlChars(data);
              const patterns = [
                /(?:xdg-open|open|start)\s+(https?:\/\/\S+)/gi,
                /OPEN_URL:\s*(https?:\/\/\S+)/gi,
                /Opening\s+(https?:\/\/\S+)/gi,
                /Visit:\s*(https?:\/\/\S+)/gi,
                /View at:\s*(https?:\/\/\S+)/gi,
                /Browse to:\s*(https?:\/\/\S+)/gi
              ];
              ...
              patterns.forEach((pattern) => {
                let match;
                while ((match = pattern.exec(sanitizedData)) !== null) {
                  urlsToOpen.add(match[1]);
                }
              });
              ...
              if (urlsToOpen.size > 0) {
                ...
                // Replace OPEN_URL echoes with a user-friendly message
                outputData = outputData.replace(/OPEN_URL:\s*(https?:\/\/\S+)/gi, (_, url) => `🌐 Opening in browser: ${url}`);
              }
